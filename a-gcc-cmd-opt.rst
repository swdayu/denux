...
**-fcall-saved-<reg>** ::
    ...
    将名为 reg 的寄存器视为一个可分配的寄存器，该寄存器会被函数保存。它可以被分配给临时
    变量或跨越调用的变量。以这种方式编译的函数如果使用了寄存器 reg，则会保存和恢复它。
    在帧指针或栈指针上使用该选项是错误的。对于在机器的执行模型中具有固定普遍角色的其他
    寄存器使用此标志也会产生灾难性后果。如果将此标志用于可能返回函数值的寄存器，会产生
    另一种灾难性后果。此标志没有负形式，因为它指定了一个三选一的选择。

**-fcall-used-<reg>** ::
    将名为 reg 的寄存器视为一个可分配的寄存器，该寄存器会被函数调用破坏。它可以被分配给
    临时变量或不会跨越调用的变量。以这种方式编译的函数不会保存和恢复寄存器。在帧指针或
    栈指针上使用该选项是错误的。对于在机器的执行模型中具有固定普遍角色的其他寄存器使用
    此标志也会产生灾难性后果。此标志没有负形式，因为它指定了一个三选一的选择。
...
**-mrtd** ::
    ...
    参数过多，将导致严重错误的代码，正常情况下通多余的参数会被无害地忽略，因为 cdecl
    调用约定是调用者清理栈参数，传多少都是调用者自己传自己清理。
